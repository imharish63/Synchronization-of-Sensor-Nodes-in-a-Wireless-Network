...
// latency is checked simultaneously of each client.
#include "WiFi.h"
#include "esp_now.h"
#include <ESP32Time.h>
...
void OnDataRecv(const uint8_t * mac_addr, const uint8_t *incomingData, int len) {
  ...
  if (len == 8){  // receive the processing time at clients during latency check
    ...
    long time_diff_2_way= ((rtc.getEpoch() - myData_send.epoch) * 1000000L) \
                          +(rtc.getMicros() - myData_send.time_us);
    ...
    if (clients[myData_rev.id - 1].iteration < 10){  // no. of iterations for which data received from a client
      ...
      Serial.printf("current i value : %d \r\n" , i);   
    }
  }
...
void loop() {
  ...
  // latency check concurrently.
    ...
    // Broadcast dummy data to clients via ESP NOW during latency check
    esp_err_t result = esp_now_send(0, (uint8_t *) &myData_send_dummy, sizeof(myData_send_dummy));
    if (result == ESP_OK) {
      Serial.println("Sent with success \r\n");
    }
    else {
...
//    delay(10); // Optional
    ...
    if (i == 40){ // this is to resync the devices after an interval
      // i can be 19 if one can go forward with the initail latency check.
      i = 9;
